...
#
#func test_duel() -> void:
	#var soldiers: Array[SoldierSimulator]
	#soldiers.append(SoldierSimulator.new(State.Soldier.ELITE, 0))
	#soldiers.append(SoldierSimulator.new(State.Soldier.ELITE, 0))
...
#
#
#func prepare_duel(soldiers_: Array[SoldierSimulator]) -> void:
	#for soldier in soldiers_:
		#soldier.stance.roll()
		#soldier.impulse.roll()
#
#func resolve_duel(soldiers_: Array[SoldierSimulator]) -> void:
	#var damage = soldiers_.front().impulse.current_face - soldiers_.back().impulse.current_face
	#if damage == 0: return
	#
	#var victim: SoldierSimulator = soldiers_.back()
	#
	#if damage < 0:
		#damage = abs(damage)
